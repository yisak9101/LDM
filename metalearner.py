...
class MetaLearner:
    ...
    def __init__(self, args):
        ...
        args.max_trajectory_len = self.envs._max_episode_steps #ex. 15 for navigation
        args.max_trajectory_len *= self.args.max_rollouts_per_task # ex 15 * 4 for navigation
    ...
    def train(self):
        ...
        for self.iter_idx in range(self.args.num_updates):
            ...
            exclude_list = np.zeros(self.args.num_processes)

            for step in range(self.args.policy_num_steps): #ex gridworld 60
        ...
        if self.iter_idx % self.args.vis_interval == 0:
            ...
            if self.args.env_name == 'GridNavi-v0': #gridworld
                ...
                # hopper-mass-inter : [0.25, 3.25] + [0.75, 1.25, 1.75, 2.25, 2.75]
                    ...
                    indistribution_avg_return = np.mean(ret_list[2:4])
                    test_avg_return = np.mean(ret_list[4:])
                elif "cheetah-vel-inter" in self.args.env_name:
                    train_avg_return = np.mean(ret_list[:2])
                    indistribution_avg_return = np.mean(ret_list[2:4])
                    test_avg_return = np.mean(ret_list[4:])
